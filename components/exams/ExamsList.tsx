...
export function ExamsList({ initialExams, onDataChanged, initialSearchTerm = '', isAdmin = false }: { initialExams: any[], onDataChanged: () => void, initialSearchTerm?: string, isAdmin?: boolean }) {
  ...
  const {
    ...
    filteredExams,
    creators
  } = useExamsList(initialExams, onDataChanged, initialSearchTerm);
      ...
      <DashboardCard>
        <div className="flex flex-col md:flex-row gap-4 mb-6 relative z-20 w-full">
          ...
          <div className="flex flex-row items-center gap-4 w-full md:w-auto shrink-0 z-10">
            ...
            <PrimaryButton onClick={() => router.push(isAdmin ? '/admin/exams/create' : '/dashboard/exams/create')} icon={<Plus className="w-4 h-4 md:w-5 md:h-5" />}>
                  ...
                  <tr key={exam.id} className="hover:bg-muted/50 transition-colors group">
                    ...
                    <td className="p-2 md:p-4 px-3 md:px-6 text-center whitespace-nowrap">
                      <ActionButtonGroup>
                        ...
                        <ActionButton onClick={() => router.push(isAdmin ? `/admin/exams/${exam.id}/edit` : `/dashboard/exams/${exam.id}/edit`)} icon={Edit} title="ویرایش" color="amber" />
                        <ActionButton onClick={() => router.push(isAdmin ? `/admin/reports?examId=${exam.id}` : `/dashboard/reports?examId=${exam.id}`)} icon={BarChart2} title="مشاهده نتایج" color="indigo" />
                  ...
                  <td colSpan={isAdmin ? 7 : 6} className="px-4 md:px-6 py-8">
                    {exams.length > 0 ? (
                      ...
                      <EmptyState 
                        ...
                        description="" 
                        action={<button onClick={() => router.push(isAdmin ? '/admin/exams/create' : '/dashboard/exams/create')} className="mt-4 text-primary hover:underline text-sm font-bold">همین الان اولین آزمون را بسازید</button>}
                      />
                    )}
                  </td>
                </tr>
