...
import { UsersPerformance } from '@/components/reports/UsersPerformance';
import { ScoreBadge } from '@/components/ui/ScoreBadge';

export function Reports({ initialExams, initialSelectedExamId, initialSubTab = 'reports' }: { initialExams?: any[], initialSelectedExamId?: string | null, initialSubTab?: 'reports' | 'users' }) {
  const [activeSubTab, setActiveSubTab] = useState<'reports' | 'users'>(initialSubTab);

  const handleSubTabChange = (tab: 'reports' | 'users') => {
    setActiveSubTab(tab);
    // Removed old onNavigate call
  };
  ...
  } = useReports(initialSelectedExamId);
    ...
    <div className="space-y-6 md:space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 relative">
      ...
      <DashboardCard className="p-4 md:p-6">
        ...
        {activeSubTab === 'reports' ? (
          ...
          <UsersPerformance initialExams={initialExams || []} />
        )}
      </DashboardCard>
