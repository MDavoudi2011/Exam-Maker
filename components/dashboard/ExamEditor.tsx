...
export function ExamEditor({ exam, initialQuestions, user }: any) {
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      if (toRemove.length > 0) {
        await supabase.from('exam_questions').delete().in('id', toRemove);
      }
      ...
      const toAdd = selectedQuestions.filter(q => !oldQuestionIds.includes(q.id)).map(q => ({
        exam_id: exam.id,
        question_id: q.id,
        order_index: 0 // Will update the order index of all in a moment
      }));

      if (toAdd.length > 0) {
        await supabase.from('exam_questions').insert(toAdd);
      }

      // now sync the order
      const { data: updatedEqs } = await supabase.from('exam_questions').select('*').eq('exam_id', exam.id);
      if (updatedEqs) {
        for (let i = 0; i < selectedQuestions.length; i++) {
          const qId = selectedQuestions[i].id;
          const eqId = updatedEqs.find(eq => eq.question_id === qId)?.id;
          if (eqId) {
             await supabase.from('exam_questions').update({ order_index: i }).eq('id', eqId);
          }
        }
      }
    ...
    return (
      <div className="flex flex-col items-center py-20 px-4 font-sans animate-in fade-in zoom-in duration-500">
        <div className="w-full max-w-4xl bg-white dark:bg-slate-900 rounded-[2rem] p-8 md:p-12 text-center shadow-2xl shadow-slate-200/50 dark:shadow-none border border-emerald-100 dark:border-emerald-900/30">
          ...
          <h1 className="text-3xl font-extrabold mb-4 text-emerald-950 dark:text-emerald-50">تغییرات آزمون با موفقیت ثبت شد!</h1>
          <p className="text-slate-500 font-medium mb-10 leading-relaxed">لینک عمومی آزمون به شرح زیر است.</p>
          ...
          <Link href="/dashboard" className="text-slate-500 hover:text-slate-800 dark:hover:text-slate-200 font-bold transition-colors">
            بازگشت به پیشخوان
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-6xl w-full mx-auto py-10 px-4 mb-20 font-sans" dir="rtl">
      
      {/* Header */}
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Link href="/dashboard" className="p-2 -mr-2 text-slate-400 hover:text-slate-800 dark:hover:text-slate-200 transition-colors">
            <ArrowRight className="w-6 h-6" />
          </Link>
          <h1 className="text-2xl md:text-3xl font-black text-slate-900 dark:text-white">
            {step === 1 ? 'ویرایش اطلاعات آزمون' : 'ویرایش سوالات'}
          </h1>
        </div>
        {step === 2 && (
