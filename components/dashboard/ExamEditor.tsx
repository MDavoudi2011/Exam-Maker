...
export function ExamEditorInner({ exam, initialQuestions, user, onNavigate, onDataChanged }: any) {
  ...
  if (step === 3) {
    return <ExamFormStep3 publishUrl={publishUrl} copied={copied} copyLink={copyLink} onCreated={() => onNavigate && onNavigate("exams")} title="تغییرات آزمون با موفقیت ثبت شد!" subtitle="لینک عمومی آزمون به شرح زیر است." />;
  }
      ...
      <div className="bg-card dark:bg-background rounded-[2rem] shadow-xl shadow-black/5 border border-border overflow-hidden">
        {step === 1 && (
          <ExamFormStep1
            ...
            secondaryButton={
              <button onClick={() => onNavigate && onNavigate("exams")} className="bg-card hover:bg-muted/50 dark:bg-background border border-border text-foreground dark:text-muted-foreground h-12 px-6 rounded-xl text-sm font-bold flex items-center justify-center gap-2 shadow-sm transition-all">
